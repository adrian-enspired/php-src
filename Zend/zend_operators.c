...
ZEND_API zend_result ZEND_FASTCALL mod_euc_function(zval *result, zval *op1, zval *op2) /* {{{ */
{
	zend_long op1_lval, op2_lval, mod;

	convert_op1_op2_long(op1, op1_lval, op2, op2_lval, result, ZEND_MOD_EUC, "%%");
	...
	mod = op1_lval % op2_lval;
	if (mod < 0) {
		/* Euclidean modulo: result is always in [0, |op2|) */
		mod += (op2_lval < 0) ? -op2_lval : op2_lval;
	}
